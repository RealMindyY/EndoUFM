...
class ResBottleneckBlock_DSConv(nn.Module):
    """
    The standard bottleneck residual block without the last activation layer.
    It contains 3 conv layers with kernels 1x1, 3x3, 1x1.
    """
    ...
    def __init__(
        ...
        # self.conv3 = nn.Conv2d(bottleneck_channels, out_channels, 1, bias=False)
        self.norm3 = LayerNorm(bottleneck_channels)
        self.act3 = act_layer()
        self.norm4 = LayerNorm(out_channels)
        ...
        self.conv4 = nn.Conv2d(bottleneck_channels, out_channels, 1, bias=False)
        
        for layer in [self.conv1, self.conv2, self.conv3,self.conv4]:
            weight_init.c2_msra_fill(layer)
        for layer in [self.norm1, self.norm2,self.norm3]:
            layer.weight.data.fill_(1.0)
            layer.bias.data.zero_()
        # zero init last norm layer.
        self.norm4.weight.data.zero_()
        self.norm4.bias.data.zero_()
