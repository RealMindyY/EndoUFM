...
class RVLinear(nn.Linear, LoRALayer):
    # LoRA implemented in a dense layer
    def __init__(
        self, 
        in_features: int, 
        ...
        fan_in_fan_out: bool = False, # Set this to True if the layer to replace stores weight like (fan_in, fan_out)
        merge_weights: bool = False,
        random1:torch.tensor = 0,
        random2:torch.tensor = 0,
        **kwargs
    ):
        nn.Linear.__init__(self, in_features, out_features, **kwargs)
        LoRALayer.__init__(self, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout,
        ...
        self.random_1=random1
        self.random_2=random2
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'lora_A'):
            ...
            nn.init.kaiming_uniform_(self.lora_V, a=math.sqrt(5))
            # 消融实验，kaiming_norm
            # nn.init.kaiming_normal_(self.lora_U, a=math.sqrt(5))
            # nn.init.kaiming_normal_(self.lora_V, a=math.sqrt(5))
            # 消融实验，kaiming_norm
            # nn.init.uniform_(self.lora_U)
            # nn.init.uniform_(self.lora_V)
    ...
    #         nn.init.zeros_(self.lora_B)
    #         with torch.no_grad():
    #             self.lora_U.copy_(self.random_1)
    #             self.lora_V.copy_(self.random_2)
    ...
    #         # nn.init.kaiming_uniform_(self.lora_A, a=math.sqrt(5))
    #         with torch.no_grad():
    #             self.lora_A.copy_(self.random_1)
    #             self.lora_B.copy_(self.random_2)
    #         nn.init.ones_(self.lora_U)
