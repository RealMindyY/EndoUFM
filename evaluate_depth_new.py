...
# splits_dir = "/hdd2/yxn/code/depth_model/splits"
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.eval_split == 'endovis':
            ...
            filenames = readlines(os.path.join(splits_dir, opt.eval_split, "test_files_20.txt"))
            dataset = datasets.hamlynRAWDataset(opt.data_path, filenames, 
                                                opt.height, opt.width,
                                                [0], 4, is_train=False)
        elif opt.eval_split == 'SERV-CT':
            filenames = readlines(os.path.join(splits_dir, opt.eval_split, "test_files.txt"))
            dataset = datasets.CTRAWDataset(opt.data_path, filenames, 
                                                opt.height, opt.width,
                                                [0], 4, is_train=False)
            # MAX_DEPTH = 25000
        elif opt.eval_split == 'endonerf':
            filenames = readlines(os.path.join(splits_dir, opt.eval_split, "test_files.txt"))
            dataset = datasets.NERFRAWDataset(opt.data_path, filenames, 
                                                opt.height, opt.width,
                                                [0], 4, is_train=False)
        ...
        if opt.model_type == 'endoufm':
            depther = endoufm.endoufm(
                backbone_size = "base", r=opt.lora_rank, lora_type=opt.lora_type,
                image_shape=(224,280), pretrained_path=opt.pretrained_path, # hamlyn 252
    ...
    if opt.eval_split == 'endovis':
        ...
        gt_path = os.path.join(splits_dir, opt.eval_split, "gt_depths_20.npz")
        gt_depths = np.load(gt_path, fix_imports=True, encoding='latin1')["data"]
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader)):
            ...
            if opt.ext_disp_to_eval is None:
                ...
                if opt.model_type == 'endoufm' or opt.model_type == 'afsfm':
            ...
            if opt.eval_split == 'endovis':
                # gt_depth = gt_depths[i].astype(np.float32) / 10
                gt_depth = gt_depths[i]
            elif opt.eval_split == 'hamlyn':
                gt_depth = gt_depths[i]
            elif opt.eval_split == 'c3vd' or opt.eval_split == 'SERV-CT' or opt.eval_split == 'endonerf':
    ...
    #     cls.append(cl[1])
    # cls = np.array(cls)
    print("\n       " + ("{:>11}      | " * 8).format("abs_diff","abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3"))
